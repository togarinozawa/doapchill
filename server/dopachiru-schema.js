 ...
 * ドパチル同期用のテーブル。
 *
 * スキマス側の流儀に合わせて `CREATE TABLE IF NOT EXISTS` だけで組んであり、
 * 何度呼んでも安全です。マイグレーションフレームワークは使いません。
 *
 * ## 置き場所について
 * `src/db.js` の CREATE TABLE 群に貼っても、この関数を呼ぶだけでも構いません。
 * routes/dopachiru.js が読み込み時に自分で呼ぶので、**何もしなくても動きます**。
 * 二重に呼んでも IF NOT EXISTS なので害はありません。
 *
 * ## user_id について
 * いまのスキマスは実質単一ユーザー(JWT の userId は 1 のハードコード)ですが、
 * 列だけ先に持たせてあります。あとでマルチユーザー化するとき、
 * 列を足し直すより楽なので。
 */

function initDopachiruSchema(db) {
  db.exec(`
    ...
    CREATE TABLE IF NOT EXISTS dopachiru_entities (
      ...
      kind       TEXT    NOT NULL,              -- 'rules' | 'tags' | 'gates' | 'changeRequests'
  ...
  // 版数の行が無ければ作る
  db.prepare(
    'INSERT OR IGNORE INTO dopachiru_meta (user_id, rev) VALUES (?, 0)'
  ).run(1);
}

module.exports = { initDopachiruSchema };
