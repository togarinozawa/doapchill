 ...
 * ## なぜルールの画面ではなくここなのか
 *
 * 決めたくなるのは**使う直前**だからです。「今日はこれで行く」と思った瞬間に
 * ルール一覧を開いて、雛形を選んで、条件を組んで……とやるうちに、
 * 決める気は消えます。
 *
 * ## なぜ明日には消えるのか
 *
 * 残り続けると分かっているものは、作る前に「これを一生守れるか」を考えることに
 * なります。その場では作られません。明日消えると分かっていれば、いま決められる。
 *
 * 中身はただのルールなので、気に入ったらルールの画面で期限を外す……ではなく、
 * 同じものを普通のルールとして作り直してください。**その場の枠は、その場のもの。**
 */
@Composable
fun OneShotCard() {
        ...
        Column(Modifier.padding(16.dp)) {
            ...
            Row(horizontalArrangement = Arrangement.spacedBy(8.dp)) {
                Button(
                    ...
                    onClick = {
                        val rule = OneShotLimit.build(
                            target = Target(packages = packages),
                            useMinutes = useMinutes,
                            restMinutes = restMinutes,
                            expiresAtSec = OneShotLimit.endOfDay(LocalDateTime.now()),
                            // この端末だけ。「今日はこれで行く」はこの端末の話で、
                            // 同じ名前のアプリが PC にもあると巻き添えになる
                            devices = setOfNotNull(DopaRuntime.myDeviceId.takeIf { it.isNotBlank() }),
                        )
                        scope.launch {
                            // 縛りを増やす側なので関門は通さない(雛形と同じ)
...
private fun TemporaryRuleRow(
    ...
    Row(Modifier.fillMaxWidth(), verticalAlignment = Alignment.CenterVertically) {
        Column(Modifier.weight(1f)) {
            Text(
                rule.target.packages.joinToString("・") { label(it) }.ifBlank { rule.name },
