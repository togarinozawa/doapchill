...
**「閉じたあと開けない」を入れるかどうか**の1択に畳んであります。

| UI の選択 | 内部 |
|---|---|
| 閉じる・やんわり | `block`(押し切れる) |
| 閉じる・しっかり | `block`(押し切れない) |
| 閉じる + 閉じたあと開けない(N分) | `lockout`(N分) |
| 少し待たせて通す | `delay` |
| 警告だけ | `warn` |
...
| `block` | 閉じる(条件が続くあいだ。やんわり/しっかり) | 100 |
| `lockout` | 閉じて、N分は開けない(押し切れない) | 200 |
