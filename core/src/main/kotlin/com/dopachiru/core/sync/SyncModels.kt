...
object SyncKinds {
     ...
     *
     * ルールは端末ごとに直接作り、ほかの端末のルールは名札([RULE_CATALOGS])から
     * 選んで連動させます([com.dopachiru.core.condition.types.LinkedRuleCondition])。
     * 使った時間までは配りません。スマホで持ち時間を使い切っても
    ...
    /**
     * 端末ごとのルールの名札([RuleCatalog])。
     *
     * ルールは配らないので、ほかの端末のルールを予約や連動で指すにはこれが要ります。
     * 鍵は deviceId で、1台ぶんをまるごと置き換えます。
     */
    const val RULE_CATALOGS = "ruleCatalogs"
